...
(define make-array-list
 (lambda L
  ...
  (letrec  ; helper procedures. We will add some!
    ...
    (double-capacity)))]
     ...
     [copy-contents (lambda (from-vec to-vec last)
      (if (>= last 0)
       ...
       [shift-right (lambda (index end)
        (if (not (equal? end index))
         (begin
          (vector-set! v end (vector-ref v (- end 1)))
          (shift-right index (- end 1))
         )))]
   ...
   ;     (copy-contents temp v (- size 1)))
        ;(let* ([index (cadr args)]
        ;[temp-val (vector-ref v index)]
        ;[temp-vec v])
        ;(shift-right (cadr args) size (copy-contents v (make-vector size) (- size 1)))
       (shift-right (cadr args) size)
       (vector-set! v (cadr args) (car args)))
       ;)
   ...
   ;Remove the quotes when you are ready to use these testcases.
